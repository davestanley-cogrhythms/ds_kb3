...
function plot_unit_summary(currspike,curr_stage,ctgsetli,filename,unitname,curr_unit,plot_sfc_on,fsavepath,fsavename)
    display_on = 1;
    
    if display_on
        figure('Position',[240    86   990   712],'Visible','on'); 
    else
        figure('Position',[240    86   990   712],'Visible','off');
    end
    ...
    legend([line1 line2], 'CtgA1','CtgA2');
    ...
    add_stages; xlabel('Time(ms)');ylabel('Spikes/s');
    legend([line1 line2], 'CtgB1','CtgB2');
    ...
    if ~display_on
        mkdir_dave(fsavepath);
        print(gcf,fullfile(fsavepath,fsavename),'-dpng');
        close all;
    end
...
function plot_raster_component(currspike_ctg,curr_stage,start_row,color)
    ...
    [boundsir] = get_stagesir(curr_stage,0,0);
...
function [line] = plot_raster_mean(currspike_ctg,curr_stage,color)
    ...
    mylims = [0 max(smCS)]*2;
    if diff(mylims) ~= 0    % Make sure that mylims isn't [0 0]
        ylim(mylims); 
    end
...
function add_stages
    ...
    for i = 1:length(stages_of_interest)
        ...
        [boundsir, name] = get_stagesir(stage,0,0);
