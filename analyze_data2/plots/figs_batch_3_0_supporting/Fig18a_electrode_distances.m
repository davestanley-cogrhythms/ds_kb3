...
function [wrkspc_buffer, out] = Fig18a_electrode_distances(wrkspc_buffer,s,opts_exclude,opts_pls,opts_perm,do_AB_symmetry)
...
%             i=i+1; group(i)=grt; group(i).criteria(5)=[0]; group(i).ctgs=5;   % Ctg1-2 non-deciders
            ...
            % Merge schemes A & B
            if do_AB_symmetry
                group2 = [group_merge(group(1:3)) group(4)];
                group = group2;
                clear group2
            else
                group = group(2:4);
...
if plot_on_electrode_distances
    [hsp, group_out] = plot_electrode_distances(group(1:end),md,mypairs,bad_any);
    
    compare_distances = 1;
    if compare_distances
        opts_PSC.remove_dependent = 1;
        fign; [h1] = plot_stats_custstruct(group_out,opts_PSC);
    end
end
