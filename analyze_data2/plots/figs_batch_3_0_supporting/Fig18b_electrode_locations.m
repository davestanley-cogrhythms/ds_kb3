...
function [wrkspc_buffer, out] = Fig18b_electrode_locations(wrkspc_buffer,s,opts_exclude,opts_pls,opts_perm)
...
%             i=i+1; group(i)=grt; group(i).criteria(5)=[0]; group(i).ctgs=5;   % Ctg1-2 non-deciders
        ...
        case {5,6}      % Different group for each day!
            ...
            % Map sp's as needed
            [sp1] = map_sp(perm_mode, sfc_mode,out_perm.mypairs,mypairs,sp,wrkspc_buffer.currmd.md,bad_any_perm,bad_any,sp_threshold);
            ...
            crit_sp = 2*ones(1,size(sp1,2));
            crit_sp(1) = 1;
            crit_all_days = logical(eye(length(idsu)));
            for i = 1:Ndays
                if groupmode == 5
                    error('unfinished');
                    group(i)=grt; group(i).criteria = [crit_sp crit_all_days(i,:)]; group(i).ctgs = 1; group(i).legend = [funames1D{find(sp(:,i),1,'first')} ' (' num2str(i) ')'];
                elseif groupmode == 6
                    error('Unfinished');
                    group(i)=grt; group(i).criteria = [crit_sp crit_all_days(i,:)]; group(i).ctgs = 2; group(i).legend = [funames1D{find(sp(:,i),1,'first')} ' (' num2str(i) ')'];
                end
            end
