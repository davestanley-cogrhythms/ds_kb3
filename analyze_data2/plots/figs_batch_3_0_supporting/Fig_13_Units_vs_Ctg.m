function [wrkspc_buffer, group] = Fig_13_Units_vs_Ctg(wrkspc_buffer,sfc_mode,curr_stage_sfc,freqband_stats,opts_exclude,opts_pls,perm_mode,curr_stage_sp,freqband_stats_perm,opts_perm,plot_on_func,do_units)
    ...
    i=0;
    i=i+1;  group(i).legend = ['$\rm \bf C/D \thinspace ' pername '  $']; 
    i=i+1;  group(i).legend = ['$\rm \bf \overline{C/D} \thinspace ' pername ' $'];
    i=i+1;  group(i).legend = ['$\rm \bf F/T \thinspace ' pername ' $']; 
    i=i+1;  group(i).legend = ['$\rm \bf \overline{F/T} \thinspace ' pername ' $'];
