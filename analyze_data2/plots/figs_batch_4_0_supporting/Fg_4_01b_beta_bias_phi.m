function [wrkspc_buffer, group] = Fg_4_01b_beta_bias_phi(wrkspc_buffer,sfc_mode, perm_mode,opts_exclude, opts_perm,freqband_stats,freqband_stats_perm,groupmode)
...
%     opts_exclude.remove_dependent = 0;       % Remove dependent electrode pairs for FFC
    ...
    % More pls switches
    opts_pls = Opts_Pls;
    opts_pls.plotmode = 8;                   % 1-SFC; 2-PSD; 3-time series; 4-phase; 5-SpkPSD; 6- Abs(Cave*exp(i*phi)); 7 - angle(Cave*exp(i*phi)); 8-Cave*exp(i*phi))
...
%             i=i+1; group(i)=grt; group(i).criteria(1:2)=[0,0]; group(i).ctgs=1;   % Ctg1-2 deciders
%             i=i+1; group(i)=grt; group(i).criteria(1:2)=[0,0]; group(i).ctgs=2;   % Ctg1-2 non-deciders
            ...
            i=i+1; group(i)=grt; group(i).criteria(1:2)=[0,1]; group(i).ctgs=2;   % Ctg1-2 non-deciders
            group = group.query_legend(group0);
...
%             i=i+1; group(i)=grt; group(i).criteria(3:4)=[0,0]; group(i).ctgs=3;   % Ctg1-2 deciders
%             i=i+1; group(i)=grt; group(i).criteria(3:4)=[0,0]; group(i).ctgs=4;   % Ctg1-2 non-deciders
            i=i+1; group(i)=grt; group(i).criteria(3:4)=[0,1]; group(i).ctgs=3;   % Ctg1-2 deciders
            i=i+1; group(i)=grt; group(i).criteria(3:4)=[0,1]; group(i).ctgs=4;   % Ctg1-2 non-deciders
            group = group.query_legend(group0);
end
...
if plot_on_real_vs_imag
    ...
    h = plot_phi_analysis(group,'plot_type',valid_plot_types{2});
