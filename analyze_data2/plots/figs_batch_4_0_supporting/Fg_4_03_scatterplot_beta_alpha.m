function [wrkspc_buffer, group] = Fg_4_03_scatterplot_beta_alpha(wrkspc_buffer,sfc_mode, perm_mode,opts_exclude, opts_perm,freqband_stats1,freqband_stats_perm1,freqband_stats2,freqband_stats_perm2,groupmode)
...
%     opts_exclude.remove_dependent = 0;       % Remove dependent electrode pairs for FFC
    ...
    opts_pls.perm2pls = 0;                   % Instead of using Cave from sfc_mode, show differences between Cave1 and Cave2 normalized by shuffle (zscore)
        ...
        opts_pls.perm2pls_allow_signed = 1;                % If 1, alows perm2pls return to be +ve or -ve; else always positive.
...
% Load pls
[wrkspc_buffer, out_pls] = load_pls(wrkspc_buffer,sfc_mode,curr_stage_sfc,freqband_stats1,opts_exclude,opts_pls);
pls1 = out_pls.pls;
pls_stats1 = out_pls.pls_stats;

[wrkspc_buffer, out_pls] = load_pls(wrkspc_buffer,sfc_mode,curr_stage_sfc,freqband_stats2,opts_exclude,opts_pls);
pls2 = out_pls.pls;
...
pls = cat(3,pls1,pls2);         % 1-5 alpha; 6-10 beta
...
% Load sp's
[wrkspc_buffer, out_perm] = load_pr(wrkspc_buffer,perm_mode,curr_stage_sp,freqband_stats_perm1,bad_any,opts_perm,opts_exclude);
...
sp = cat(2,sp1,sp2);         % 1-5 alpha; 6-10 beta
...
% Run a simple test
clear group
do_average_of_scheme = 1;
switch groupmode
%     case 0      % Plot all schemes
%         i=0;
%         % Survey A & B
%         i=i+1; group(i)=grt; group(i).criteria(1)=[1]; group(i).ctgs=1;   % Ctg1-2 deciders
%         i=i+1; group(i)=grt; group(i).criteria(2)=[1]; group(i).ctgs=1;   % Ctg1-2 non-deciders
%         i=i+1; group(i)=grt; group(i).criteria(3)=[1]; group(i).ctgs=2;   % Ctg1-2 deciders
%         i=i+1; group(i)=grt; group(i).criteria(4)=[1]; group(i).ctgs=2;   % Ctg1-2 non-deciders
%     case 1
%         i=0;
%         % Sch A
...
%         i=0;
%         % Sch B
%         i=i+1; group(i)=grt; group(i).criteria(3:4)=[1,0]; group(i).ctgs=2;   % Ctg1-2 deciders
%         i=i+1; group(i)=grt; group(i).criteria(3:4)=[0,0]; group(i).ctgs=2;   % Ctg1-2 non-deciders
%         i=i+1; group(i)=grt; group(i).criteria(3:4)=[0,1]; group(i).ctgs=2;   % Ctg1-2 deciders
%         i=i+1; group(i)=grt; group(i).criteria(3:4)=[0,0]; group(i).ctgs=2;   % Ctg1-2 non-deciders
    case 3
        
%         i=0;
%         % Sch A alpha vs beta
%         i=i+1; group(i)=grt; group(i).criteria([1,5])=[1,0]; group(i).ctgs=1;   % Ctg1-2 deciders
%         i=i+1; group(i)=grt; group(i).criteria([1,5])=[0,1]; group(i).ctgs=5;   % Ctg1-2 deciders
%         i=i+1; group(i)=grt; group(i).criteria([1,5])=[1,1]; group(i).ctgs=1;   % Ctg1-2 deciders
%         group = group.query_legend(group0);
%         
        
        i=0;
        % Sch A alpha vs beta
        i=i+1; group(i)=grt; group(i).criteria([1:2,11:12])=[1,0,0,0]; group(i).ctgs=1;   % Ctg1-2 deciders
        i=i+1; group(i)=grt; group(i).criteria([1:2,11:12])=[0,1,0,0]; group(i).ctgs=1;   % Ctg1-2 deciders
        i=i+1; group(i)=grt; group(i).criteria([1:2,11:12])=[0,0,1,0]; group(i).ctgs=6;   % Ctg1-2 deciders
        i=i+1; group(i)=grt; group(i).criteria([1:2,11:12])=[0,0,0,1]; group(i).ctgs=6;   % Ctg1-2 deciders
        i=i+1; group(i)=grt; group(i).criteria([1:2,11:12])=[1,0,1,0]; group(i).ctgs=1;   % Ctg1-2 deciders
        i=i+1; group(i)=grt; group(i).criteria([1:2,11:12])=[0,1,0,1]; group(i).ctgs=1;   % Ctg1-2 deciders
        i=i+1; group(i)=grt; group(i).criteria([1:2,11:12])=[1,0,0,1]; group(i).ctgs=1;   % Ctg1-2 deciders
        i=i+1; group(i)=grt; group(i).criteria([1:2,11:12])=[0,1,1,0]; group(i).ctgs=1;   % Ctg1-2 deciders
        
        group = group.query_legend(group0);
        i=0;
        i=i+1; group(i).legend='Alpha Cat';
        i=i+1; group(i).legend='Alpha Dog';
        i=i+1; group(i).legend='Beta Cat';
        i=i+1; group(i).legend='Beta Dog';
        i=i+1; group(i).legend='2 or more';
        
        group2 = [group(1:4), group_merge(group(5:8))];     % Merge all "and" units into a single group
        group = group2;
        
    case 4
%         i=0;
%         % Sch B alpha vs beta
%         i=i+1; group(i)=grt; group(i).criteria([2,6])=[1,0]; group(i).ctgs=2;   % Ctg1-2 deciders
%         i=i+1; group(i)=grt; group(i).criteria([2,6])=[0,1]; group(i).ctgs=6;   % Ctg1-2 deciders
%         i=i+1; group(i)=grt; group(i).criteria([2,6])=[1,1]; group(i).ctgs=2;   % Ctg1-2 deciders
%         group = group.query_legend(group0);


        i=0;
        % Sch A alpha vs beta
        i=i+1; group(i)=grt; group(i).criteria([3:4,13:14])=[1,0,2,2]; group(i).ctgs=2;   % Ctg1-2 deciders
        i=i+1; group(i)=grt; group(i).criteria([3:4,13:14])=[0,1,2,2]; group(i).ctgs=2;   % Ctg1-2 deciders
        i=i+1; group(i)=grt; group(i).criteria([3:4,13:14])=[2,2,1,0]; group(i).ctgs=7;   % Ctg1-2 deciders
        i=i+1; group(i)=grt; group(i).criteria([3:4,13:14])=[2,2,0,1]; group(i).ctgs=7;   % Ctg1-2 deciders
        i=i+1; group(i)=grt; group(i).criteria([3:4,13:14])=[1,0,1,0]; group(i).ctgs=3;   % Ctg1-2 deciders
        i=i+1; group(i)=grt; group(i).criteria([3:4,13:14])=[0,1,0,1]; group(i).ctgs=3;   % Ctg1-2 deciders
        i=i+1; group(i)=grt; group(i).criteria([3:4,13:14])=[1,0,0,1]; group(i).ctgs=3;   % Ctg1-2 deciders
        i=i+1; group(i)=grt; group(i).criteria([3:4,13:14])=[0,1,1,0]; group(i).ctgs=3;   % Ctg1-2 deciders

        group = group.query_legend(group0);
        i=0;
        i=i+1; group(i).legend='Alpha Fat';
        i=i+1; group(i).legend='Alpha Thin';
        i=i+1; group(i).legend='Beta Fat';
        i=i+1; group(i).legend='Beta Thin';
...
group = get_grouped_cells_and_data(group,sp,pls,abscissa,mypairs,bad_any,opts_pls.plotmode,freqband_stats1,funames1D,abscissa2);
%group2 = get_grouped_cells_and_data(group,sp,pls,abscissa,mypairs,bad_any,opts_pls.plotmode,freqband_stats2,funames1D,abscissa2);

%group = [group1(1), group2(2), group1(3)];
...
if plot_on_scatter
    ...
    xlabel(['FFC ' group(2).legend '-' group(1).legend]);
    ylabel(['FFC ' group(4).legend '-' group(3).legend]);
