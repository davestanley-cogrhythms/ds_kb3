function [wrkspc_buffer, group] = Fg_4_05_FFC_perm_split_plusminus(wrkspc_buffer,sfc_mode,perm_mode,curr_stage_sfc,curr_stage_sp,opts_exclude,groupmode);
...
%     freqband_stats_perm = [80 100];
    ...
    opts_pls.perm2pls = 1;                   % Instead of using Cave from sfc_mode, show differences between Cave1 and Cave2 normalized by shuffle (zscore)
        ...
        opts_pls.perm2pls_split_plusminus = groupmode;             % 0-Return everything; 1-return pos+negative in separate columns; 2-return only positive (pls.*Ramp(Diff)); 3-return only negative (pls.*Ramp(-Diff)); 4-return only significant cells (doesn't really have a use now)
...
switch groupmode
    case 0
        ...
        group(1).colour_desired = 'k';
        group(2).colour_desired = 'k';
    ...
    case 1
        ...
        grt = group0(1);
        grt.criteria = mycrit; grt.criteria_alt = []; grt.criteria_sfc = []; grt.ctgs = 1; grt.zlims_desired = [];

        % Run a simple test
        clear group
        i=0;
        i=i+1; group(i)=grt; group(i).ctgs=i; group(i).legend = 'Cat'; group(i).colour_desired = 'b';
        i=i+1; group(i)=grt; group(i).ctgs=i; group(i).legend = 'Dog'; group(i).colour_desired = 'r';
        i=i+1; group(i)=grt; group(i).ctgs=i; group(i).legend = 'Goc'; group(i).colour_desired = 'r';
        i=i+1; group(i)=grt; group(i).ctgs=i; group(i).legend = 'Tad'; group(i).colour_desired = 'b';
        i=i+1; group(i)=grt; group(i).ctgs=i; group(i).legend = 'Cat irr';
        i=i+1; group(i)=grt; group(i).ctgs=i; group(i).legend = 'Dog irr';
        i=i+1; group(i)=grt; group(i).ctgs=i; group(i).legend = 'Fat irr';
        i=i+1; group(i)=grt; group(i).ctgs=i; group(i).legend = 'Thin irr';
        i=i+1; group(i)=grt; group(i).ctgs=i; group(i).legend = 'SchA';
        i=i+1; group(i)=grt; group(i).ctgs=i; group(i).legend = 'SchB';
