...
function [wrkspc_buffer, out] = Fg_5_01c_FFC_bias_grouped_by_unit_bias(wrkspc_buffer,s,opts_exclude,opts_pls,opts_perm,second_difference_mode,prediff_subplots)
...
switch second_difference_mode
    
    case 1
                                    % Subtracts the mean of group(2) from group(1); this in effect performs the differnece operation
                                    % Have to do this rather than group_do_merge since N is not the same
        clear group2
        for i = 1:floor(length(group)/2)
            data1 = group(2*i-1).data;
            data2 = group(2*i).data;
            
            group2(i) = group(2*i-1);
            group2(i).data = mean(data1,2)- mean(data2,2);
        end
    ...
    case 2              % P values
        ...
        for i = 1:floor(length(group)/2)
            ...
            sz=size(data1); data1 = mat2cell(data1,ones(1,sz(1)),sz(2),ones(1,sz(3)));
            sz=size(data2); data2 = mat2cell(data2,ones(1,sz(1)),sz(2),ones(1,sz(3)));
            
            group2(i) = group(2*i-1);
            group2(i).data = cellfun(@ranksum,data1,data2) < 0.01;
        end
        
        clear data1 data2
        
    case 3              % z-scores assuming Gaussian
        clear group2
        for i = 1:floor(length(group)/2)
            data1 = group(2*i-1).data;
            data2 = group(2*i).data;
            
            data_std = sqrt(var(data1,[],2) + var(data2,[],2));       % Assuming normality....
            
            group2(i) = group(2*i-1);
            group2(i).data = mean(data1,2)- mean(data2,2) ./ data_std;
    ...
    case 4              % Difference normalized by the mean of permuted distribution of the absolute difference.
        operation = 4;  % Bootstrap
        group2 = grouppairs_merge(group,opts_pls.perm2pls_dophi,operation);
end

if prediff_subplots && second_difference_mode > 0
    group = [ group, group2 ];
    clear group2
elseif second_difference_mode > 0
    group = group2;
    clear group2
end
