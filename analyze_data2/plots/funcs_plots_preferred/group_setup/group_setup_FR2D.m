...
function [gr, mask] = group_setup_FR2D(gr)
    % Group coordinates are a 2D matrix ordered based on relevance of
    % changes in firing rate significances

    N_ctgs_base = get_Nctgs_base;
    N_ctgs_extras = get_Nctgs_extra;
    
    % Cell selection preset
    N_pairs_extra = round(N_ctgs_extras / 2);

    % Trial selection preset
    %all_correct_trials = [1 2 3 4];
    %all_correct_trials = [N_ctgs_base + N_ctgs_extras + 1];
    all_correct_trials = 11;
    ...
    for i = 1:Ngrall
        ...
        gr.all(i).criteria = [coords2criteria(gr.all(i).coords, N_ctgs_extras)];
    ...
    gr.all(1).legend = 'Multitasker';
    gr.all(2).legend = 'Sch B';
    gr.all(3).legend = 'Sch A';
    gr.all(4).legend = 'No preference';
