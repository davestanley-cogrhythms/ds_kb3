...
%     run_allstages(2.201711101);
%     run_allstages(2.201701101);
...
%     
      ...
      run_allstages(23.451411102);
      run_allstages(23.441411102);
      
%       run_allstages(23.441411103);
      
      %run_allstages(23.451411103);
    ...
    switch server_mode
        case 1
            if i0j0_split == 0
                for ind = 1:length(file_list)
                    ...
                    system(['qsub -l h_rt=' num2str(myhours) ':30:00 run_standalone_job.sh run_analysis_cells_binwrapp ' num2str(sfc_mode,'%10.12f') ' ' num2str(curr_stage) ' ' num2str(ind) ' ' num2str(ind) ' "' i0_str '" ' num2str(i0j0_split) '']);
                    fprintf('\n');
                end
            elseif i0j0_split == 1
                for ind = 1:actual_number_of_nodes
                    ...
                    system(['qsub -l h_rt=' num2str(myhours) ':30:00 run_standalone_job.sh run_analysis_cells_binwrapp ' num2str(sfc_mode,'%10.12f') ' ' num2str(curr_stage) ' ' num2str(pairsstart) ' ' num2str(pairsstop) ' "' i0_str '" ' num2str(i0j0_split) '']);
                    fprintf('\n');
                end
            end
        ...
        case 2
            if i0j0_split == 0
                for ind = 1:length(file_list)
                    fprintf(['Running file ' num2str(ind) '\n']);
                    system(['qsub -l h_rt=' num2str(myhours) ':30:00 -l mem_total=95G matlab_single_node_batch.sh "setup_paths_n_run(@run_analysis_cells,' num2str(sfc_mode,'%10.12f') ',' num2str(curr_stage) ',' num2str(ind) ',' num2str(ind) ',' i0_str ',' num2str(i0j0_split) ')" localOutput']);
                    fprintf('\n');
                end
            elseif i0j0_split == 1
                for ind = 1:actual_number_of_nodes
                    ...
                    pairsstop = min(Nsimulations,ind*sims_per_node);
                    fprintf(['Running pairs ' num2str(pairsstart) ' to ' num2str(pairsstop) '\n']);
                    system(['qsub -l h_rt=' num2str(myhours) ':30:00 matlab_single_node_batch.sh "setup_paths_n_run(@run_analysis_cells,' num2str(sfc_mode,'%10.12f') ',' num2str(curr_stage) ',' num2str(pairsstart) ',' num2str(pairsstop) ',' i0_str ',' num2str(i0j0_split) ')" localOutput']);
                    fprintf('\n');
                end
            end
        case 3
            if i0j0_split == 0
                for ind = 1:length(file_list)
                    fprintf(['Running file ' num2str(ind) '\n']);
                    setup_paths_n_run(@run_analysis_cells,sfc_mode,curr_stage,ind,ind,i0_str,i0j0_split);
                end
            elseif i0j0_split == 1
                for ind = 1:actual_number_of_nodes
                    pairsstart = (ind-1)*sims_per_node + 1;
                    pairsstop = min(Nsimulations,ind*sims_per_node);
                    fprintf(['Running pairs ' num2str(pairsstart) ' to ' num2str(pairsstop) '\n']);
                    setup_paths_n_run(@run_analysis_cells,sfc_mode,curr_stage,pairsstart,pairsstop,i0_str,i0j0_split);
                end
            end
        case 4
            for ind = 1:length(file_list)
                fprintf(['Running file ' num2str(ind) '\n']);
                system(['qsub -l h_rt=' num2str(myhours) ':30:00 -l gpus=1 matlab_single_node_batch.sh "setup_paths_n_run(@run_analysis_cells,' num2str(sfc_mode,'%10.12f') ',' num2str(curr_stage) ',' num2str(ind) ',' num2str(ind) ',' i0_str ',' num2str(i0j0_split) ')" localOutput']);
                fprintf('\n');
            end
    end
