...
%         March 27 2014 - Updated to allow t to be a vector (Dave)
    ...
    for i = 1:length(cols)
        hl{i} = fname(t(:,cols(i)),squeeze(X(:,cols(i),:)),LineSpec{:}); hold on; 
        if showErrorbars
            
            % Massage LineSpec to get it in the form we want
            LineSpec2=LineSpec;
            lsc_ind = find(strcmp(LineSpec,'Color'));
            if ~isempty(lsc_ind)
                if ~ischar(LineSpec{lsc_ind+1})
                    LineSpec2=LineSpec          % The color specification is a row vector
                    LineSpec2{lsc_ind} = 'cmap';
                end
            end
            
            
            hold on; hl{i} = boundedline(t,squeeze(X),permute(squeeze(Xste),[1 3 2]),'alpha',LineSpec2{:});
