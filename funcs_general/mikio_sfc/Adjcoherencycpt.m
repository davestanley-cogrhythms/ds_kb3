...
if trialave; S12=squeeze(mean(S12,2)); S1=squeeze(mean(S1,2)); S2=squeeze(mean(S2,2)); Msp2=mean(Msp2);end;
R=Msp2;c0 = StarRate/rate;

C12=S12./sqrt(S1.*S2)./sqrt((1-c0)*rate./S2./c0+1);
C=abs(C12);%original Chronux code
% C12=S12./sqrt(S1.*(c0*(1-c0)*rate+c0^2.*S2)).*c0;
% C12=S12./sqrt(S1.*(c0*(1-c0)*rate+c0^2.*S2)).*c0;

C=abs(C12);%Mikio's code

phi=angle(C12);
if nargout==10; 
  if fscorr==1; 
     [confC,phistd,Cerr]=Adjcoherr(C,J1,J2,err,trialave,[],Nsp2,c0,rate,Fs); 
  else
     [confC,phistd,Cerr]=Adjcoherr(C,J1,J2,err,trialave); 
  end;
elseif nargout==9;
  if fscorr==1; 
     [confC,phistd]=Adjcoherr(C,J1,J2,err,trialave,[],Nsp2,c0,rate,Fs); 
  else
     [confC,phistd]=Adjcoherr(C,J1,J2,err,trialave,[],[],c0,rate,Fs); 
  end;
end;
clear Msp2
