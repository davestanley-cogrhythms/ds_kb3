...
function [tr_match,tr_nonmatch] = decode_match_nonmatch(md)

    % Jefferson's way... much simpler

    tr_match = logical(mod(md.each_trial_condition,2));     % Even
    tr_nonmatch = ~logical(mod(md.each_trial_condition,2)); % Odd
