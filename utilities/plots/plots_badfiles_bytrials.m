    ...
    % Plots bad clipping and bad 60 Hz data. Bases badclipping on the
    % number of bad trials per file, as opposed to previous metric.
    % Note that the data might already be filtered...
    ...
    % Merge files
    clear li_clip li_60 li_psd li_f
...
%     li_hist = cat(2,s_clip.Xint_hist{:});         % No longer saved
    ...
    % % For bad trials...
    li_Nbadtrials = cellfunu(@sum,bad_trials); li_Nbadtrials = cat(2,li_Nbadtrials{:});
%     Ntrials_per_file = cellfun(@(x) size(x,2),lfp_all); %For verification only
%     percent_est = li_Nbadtrials ./ Ntrials_per_file * 100;
    ...
    thresh_clip_trials = get_clipping_threshold_trials;
    thresh_60Hz = get_60Hz_threshold;
    
    %thresh_clip = 2;  % (For testing only!!!)
    %thresh_60Hz = 0.4;
    
    bad_clip = li_clip > thresh_clip_trials(2);
    ...
    if plot_on
        ...
        for i = 1:length(testlist);
            ...
            %subplotrows(2,1);
            intplot(t(plot_range),lfp_mat(plot_range,testlist(i))); title(['File #' num2str(a) 'e' num2str(b) ' ' fnames{a} ' Percent_badtrials = ' num2str(li_clip(testlist(i))) ' Nbadtrials ' num2str(li_Nbadtrials(testlist(i)))]);
